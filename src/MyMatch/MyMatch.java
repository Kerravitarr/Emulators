...
public class MyMatch {
	...
	private final static char[] crc16tab = {0x0000, 0x1021, 0x2042, 0x3063,
	...
	public static int[] calculateCRC(char[] data, int offset, int len) {
		byte data_b[] = new byte[data.length];
		for (int i = 0; i < data_b.length; i++) {
			data_b[i] = (byte) data[i];
		}
		return calculateCRC(data_b, offset, len);
	}

	// ИСИ. Вычисление суммированиеим
	public static final byte calculateCRC_Sum(byte[] data, int offset,
			int len) {
		byte crc = 0;
		for (int i = offset; i < len && i < data.length; i++) {
			crc += data[i];
	...
	// Специально для МПЦ МЗ Ф
	public static final int[] crc_16(byte[] data, int offset, int end) {
		char crc = 0xFFFF;
		for (int i = offset; i < end && i < data.length; i++) {
			crc = (char) (crc << 8
					^ crc16tab[crc >> 8 & 0xff ^ (char) data[i] & 0xFF]);
		}
		int[] crc_out = {crc >> 8 & 0xff, crc & 0xFF};
		return crc_out;
	}
	public static int[] crc_16(char[] data, int offset, int len) {
		byte data_b[] = new byte[data.length];
		for (int i = 0; i < data_b.length; i++) {
			data_b[i] = (byte) data[i];
		}
		return crc_16(data_b, offset, len);
	}
	
	public static int[] crc16_bypass(char[] data, int offset, int len) {
		byte data_b[] = new byte[data.length];
		for (int i = 0; i < data_b.length; i++) {
			data_b[i] = (byte) data[i];
		}
		return crc16_bypass(data_b, offset, len);
	}
	public static int[] crc16_bypass(byte[] data, int offset, int end) {
		int crc;
		crc = 0x0000;
		for (int i = offset; i < end && i < data.length; i++) {
			crc = (((int)data[i]) << 8) ^ crc;
			for (int y = 0; y < 8; y++) {
				if ((crc & 0x8000) != 0)
					crc = (crc << 1) ^ 0x8005;
				else
